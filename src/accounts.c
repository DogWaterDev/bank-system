...
void add_account(uuid_t *account_uuid_container, const char *name) {
	...
	account_t *tmp = &(account_list[account_count == 0 ? 0 : account_count-1]);
...
uint64_t get_account_balance(uuid_t account_uuid) {
	...
	return (uint64_t)(-1); // if someone reaches this amount normally just fuck them ngl
}
...
int get_account_negative(uuid_t account_uuid) {
	int index = get_account_index(account_uuid);
	if (index != -1) {
		return account_list[index].negative;	
	}
	return -1;
}
...
int set_account_negative(uuid_t account_uuid, int negative) {
	int index = get_account_index(account_uuid);
	if (index != -1) {
		account_list[index].negative = negative;
		return 0;
	}
	return -1;
}
